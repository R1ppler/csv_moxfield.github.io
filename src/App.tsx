...
// 1. Le decimos a TypeScript exactamente qué columnas tiene nuestra carta de Moxfield
interface CardRow {
  Name: string;
  Qty: number;
  Set: string;
  Foil: string;
  'Collector Number': string; // Lleva comillas porque tiene un espacio
  Condition: string;
  Language: string;
  'Purchase Price': string;
  Binder: string;
  'Tradelist Qty': number;
  DisplayPrice?: string | number; // Para mostrar el precio en la tabla, no es parte del CSV original
}
...
function App(): React.JSX.Element {
  ...
  const [csvList, setCsvList] = useState<CardRow[]>([]); // Array con las cartas añadidas
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [previewCard, setPreviewCard] = useState<any | null>(null);
  const [loading, setLoading] = useState<boolean>(false);
  const precioActivo = previewCard ? (isFoil ? previewCard.prices?.eur_foil : previewCard.prices?.eur) : null;

  async function buscarCartaEnScryfall() {
    try {
      setLoading(true);
      setPreviewCard(null); // Limpiar preview anterior
      let url = '';

      // Buscar por set + número (prioritario si ambos existen)
      if (cardSet && collectorNumber) {
        url = `https://api.scryfall.com/cards/${cardSet.toLowerCase()}/${collectorNumber}/${language}`;
      }
      // Buscar por nombre (descomentar si se usa el input de nombre)
      else if (cardName) {
  ...
  function añadirCarta() {
    ...
    const nuevaCarta: CardRow = {
      ...
      Binder: '',
      'Tradelist Qty': 0,
      DisplayPrice: isFoil ? previewCard.prices?.eur_foil : previewCard.prices?.eur,
    };

    setCsvList((prev) => [nuevaCarta, ...prev]);

    // Limpiar preview y buscador
    setPreviewCard(null);
    setCardName('');
    setCollectorNumber('');
    setIsFoil(false);

    // Enfocar de nuevo el set para agilizar la entrada
  ...
  function limpiarLista() {
    if (window.confirm('¿Estás seguro de que quieres limpiar toda la lista? Esta acción no se puede deshacer.')) {
      setCsvList([]);
    }
  }
  ...
  function exportarCSV() {
    ...
    const csvData = csvList.map(({ DisplayPrice, ...rest }) => rest); // Excluir DisplayPrice del CSV
    ...
    link.href = url;
    link.download = `moxfield_export_${new Date().toISOString().slice(0,10)}.csv`;
    ...
    setTimeout(() => {
      if (window.confirm('¿Quieres limpiar toda la lista? Esta acción no se puede deshacer.')) {
        setCsvList([]);
      }
    }, 500);
  }

  // OBJETIVO 3: Función para eliminar cartas de la lista
  function eliminarCarta(index: number) {
    setCsvList((prev) => prev.filter((_, i) => i !== index));
  }

  // Obtener URL de imagen (normal o cara frontal si es de doble cara)
  const getImageUrl = (cardData: any) => {
    ...
    return 'https://via.placeholder.com/488x680?text=No+Image'; // Placeholder
  };
    ...
    <div className="min-h-screen bg-slate-900 text-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-[400px]">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onKeyPress={(e) => e.key === 'Enter' && buscarCartaEnScryfall()}
                  />
                </div>
          ...
          {/* PANEL DE VISTA PREVIA Y ACCIONES (Objetivo 2) */}
          <div className={`lg:col-span-2 bg-slate-800 p-6 rounded-lg shadow-md transition-opacity duration-300 ${previewCard ? 'opacity-100' : 'opacity-50'}`}>
            ...
            {previewCard ? (
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                {/* OBJETIVO 1: Imagen reescalada (más grande) */}
                <img
                ...
                <div className="flex-1 w-full flex flex-col justify-between h-full min-h-[250px]">
                  <div>
                    ...
                    <p className="text-gray-400 uppercase text-sm">{previewCard.set_name} ({previewCard.set.toUpperCase()}) #{previewCard.collector_number}</p>
                    <p className={`text-xl mt-1 font-mono font-bold flex items-center gap-1 ${isFoil ? 'text-purple-400' : 'text-orange-300'}`}>
                      {isFoil && <span className="text-sm">✨</span>}
                      {precioActivo ? `${precioActivo} €` : '--- €'}
                    </p>
                  </div>

                  {/* Botones de acción a la derecha/abajo de la imagen */}
              ...
              <div className="flex items-center justify-center min-h-[310px] border-2 border-dashed border-slate-600 rounded-lg">
                <p className="text-gray-500 italic text-center p-4">
                  Busca una carta a la izquierda para ver la vista previa aquí.<br/>
                  Se recomienda usar Set y Número para mayor precisión (ej: 'one' y '1').
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-slate-800 p-6 rounded-lg shadow-md">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            ...
            <button
              ...
              className="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white px-5 py-2.5 rounded-lg font-bold transition flex items-center gap-2 w-full sm:w-auto justify-center"
            >
              ...
              Descargar CSV para Moxfield
            </button>

            <button
              onClick={limpiarLista}
              disabled={csvList.length === 0}
              className="bg-red-600 hover:bg-red-700 disabled:bg-gray-600 text-white px-5 py-2.5 rounded-lg font-bold transition flex items-center gap-2 w-full sm:w-auto justify-center"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {csvList.length === 0 ? (
            <div className="text-center py-12 border-2 border-dashed border-slate-700 rounded-lg">
                <p className="text-gray-400 text-base italic">No hay cartas en la lista todavía. ¡Utiliza el buscador superior!</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm text-gray-300">
                <thead className="bg-slate-700 text-gray-200 uppercase text-xs">
                  <tr>
                    ...
                    <th className="p-3 font-semibold text-center">Acciones</th> {/* OBJETIVO 3 */}
                  </tr>
                </thead>
                <tbody>
                  {csvList.map((carta, index) => (
                    <tr key={index} className="border-b border-slate-700 hover:bg-slate-750 transition-colors">
                      <td className="p-3 font-medium text-white text-base">{carta.Name}</td>
            
                      <td className="p-3 uppercase text-center font-mono bg-slate-700/30 rounded">{carta.Set}</td>
                      <td className="p-3 text-center font-mono">{carta['Collector Number']}</td>
                      <td className="p-3 text-center uppercase">{carta.Language}</td>

                      <td className="p-3 text-center text-yellow-400 text-lg">{carta.Foil ? '⭐' : ''}</td>
                      <td className="p-3 text-center font-bold">{carta.DisplayPrice ? `${carta.DisplayPrice} €` : '-'}</td>
                      {/* OBJETIVO 3: Botón eliminar */}
